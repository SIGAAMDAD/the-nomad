...
#define cvector_copy(from, to)                                            \
    do {                                                                  \
        if ((from)) {                                                     \
            ...
            N_memcpy((to), (from), cvector_size(from) * sizeof(*(from))); \
        }                                                                 \
    } while (0)
...
#define cvector_grow(vec, count)                                                      \
    do {                                                                              \
        ...
        if (vec) {                                                                    \
            ...
            void *cv_p2__ = cvector_clib_realloc(cv_p1__, cv_sz__,&cv_p2__);          \
            assert(cv_p2__);                                                          \
            (vec) = cvector_base_to_vec(cv_p2__);                                     \
        } else {                                                                      \
            void *cv_p__ = cvector_clib_malloc(cv_sz__,TAG_STATIC,&cv_p__);           \
            assert(cv_p__);                                                           \
