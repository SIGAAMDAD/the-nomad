...
void __attribute__((noreturn)) N_Error(const char* err, ...) __attribute__((format(printf, 1, 2)));
#ifndef NDEBUG
...
void idHeap::Free( void *p ) {
	...
	switch( ((byte *)(p))[-1] ) {
		...
		default: {
			N_Error( "idHeap::Free: invalid memory block" );
			break;
		}
	}
#endif
...
dword idHeap::Msize( void *p ) {
	...
	switch( ((byte *)(p))[-1] ) {
		...
		default: {
			N_Error( "idHeap::Msize: invalid memory block" );
			return 0;
		}
	}
#endif
...
void idHeap::Dump( void ) {
	...
	for ( pg = smallFirstUsedPage; pg; pg = pg->next ) {
		printf( "%p  bytes %-8d  (in use by small heap)\n", pg->data, pg->dataSize);
	}

	if ( smallCurPage ) {
		pg = smallCurPage;
		printf( "%p  bytes %-8d  (small heap active page)\n", pg->data, pg->dataSize );
	}

	for ( pg = mediumFirstUsedPage; pg; pg = pg->next ) {
		printf( "%p  bytes %-8d  (completely used by medium heap)\n", pg->data, pg->dataSize );
	}

	for ( pg = mediumFirstFreePage; pg; pg = pg->next ) {
		printf( "%p  bytes %-8d  (partially used by medium heap)\n", pg->data, pg->dataSize );
	}

	for ( pg = largeFirstUsedPage; pg; pg = pg->next ) {
		printf( "%p  bytes %-8d  (fully used by large heap)\n", pg->data, pg->dataSize );
	}
...
void Mem_DumpCompressed( const char *fileName, memorySortType_t memSort, int numFrames ) {
	...
	for ( b = mem_debugMemory; b; b = b->next ) {
		...
		if ( numFrames && b->frameNumber <  - numFrames ) {
			continue;
		}
...
void Mem_DumpCompressed_f( const idCmdArgs &args ) {
	...
	while( arg[0] == '-' ) {
		...
		if ( idStr::Icmp( arg, "s" ) == 0 ) {
			...
			printf( "memoryDumpCompressed [options] [filename]\n"
						"options:\n"
						"  -s     sort on size\n"
						"  -l     sort on location\n"
