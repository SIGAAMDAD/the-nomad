...
{
public:
	CUtlBinaryBlock( uint64_t growSize = 0, uint64_t initSize = 0 );
	~CUtlBinaryBlock()
	{
#ifdef _DEBUG
	...
	CUtlBinaryBlock( void* pMemory, uint64_t nSizeInBytes, uint64_t nInitialLength );
	CUtlBinaryBlock( const void* pMemory, uint64_t nSizeInBytes );
	CUtlBinaryBlock( const CUtlBinaryBlock& src );

	void		Get( void *pValue, uint64_t nMaxLen ) const;
	void		Set( const void *pValue, uint64_t nLen );
	const void	*Get( ) const;
	void		*Get( );

	unsigned char& operator[]( uint64_t i );
	const unsigned char& operator[]( uint64_t i ) const;

	int			Length() const;
	void		SetLength( uint64_t nLength );	// Undefined memory will result
	bool		IsEmpty() const;
	void		Clear();
	void		Purge();
...
inline uint64_t CUtlBinaryBlock::Length() const
{
	return m_nActualLength;
}

inline unsigned char& CUtlBinaryBlock::operator[]( uint64_t i )
{
	return m_Memory[i];
}

inline const unsigned char& CUtlBinaryBlock::operator[]( uint64_t i ) const
{
	return m_Memory[i];
}
...
public:
	...
	CUtlString( void* pMemory, uint64_t nSizeInBytes, uint64_t nInitialLength );
	CUtlString( const void* pMemory, uint64_t nSizeInBytes );
	...
	CUtlString &operator+=( uint64_t rhs );
	CUtlString &operator+=( double rhs );
	
	CUtlString operator+( const char *pOther ) const;

	bool MatchesPattern( const CUtlString &Pattern, uint64_t nFlags = 0 ) const;		// case SENSITIVE, use * for wildcard in pattern string

	uint64_t Format( const char *pFormat, ... );
	void SetDirect( const char *pValue, uint64_t nChars );
	...
	static uint64_t __cdecl SortCaseInsensitive( const CUtlString *pString1, const CUtlString *pString2 );
	static uint64_t __cdecl SortCaseSensitive( const CUtlString *pString1, const CUtlString *pString2 );
...
inline uint64_t __cdecl CUtlString::SortCaseInsensitive( const CUtlString *pString1, const CUtlString *pString2 )
{
	return V_stricmp( pString1->String(), pString2->String() );
}

inline uint64_t __cdecl CUtlString::SortCaseSensitive( const CUtlString *pString1, const CUtlString *pString2 )
{
...
public:
	...
	static void		 Copy( T *out_pOut, const T *pIn, uint64_t iLength );
...
public:
	...
	static void		   Copy( char *out_pOut, const char *pIn, uint64_t iLength ) { strncpy( out_pOut, pIn, iLength ); }
...
public:
	...
	static void			  Copy( wchar_t *out_pOut, const wchar_t  *pIn, uint64_t iLength ) { wcsncpy( out_pOut, pIn, iLength ); }
...
public:
	...
	uint64_t Compare( const T *rhs ) const;
	...
	// Defining AltArgumentType_t is a huint64_t to containers that they should
...
template < typename T >
uint64_t CUtlConstStringBase<T>::Compare( const T *rhs ) const
{
	if ( m_pString )
	{
		if ( rhs )
