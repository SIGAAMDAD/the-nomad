...
#ifdef AS_COMPUTED_GOTOS
#define GOTO( instr ) GOTO_ ## instr
#define NEXT_INSTRUCTION() goto *instructions[ *l_bc++ ]
#define NEXT_INSTRUCTION2() goto *instructions[ *( l_bc += 2 ) ]
#define GOTO_INSTRUCTION( a ) l_bc += ( a ); goto *instructions[ *l_bc ]
#define END()
#else
#define GOTO( instr ) case instr
#define NEXT_INSTRUCTION() l_bc++
#define NEXT_INSTRUCTION2() l_bc += 2
#define GOTO_INSTRUCTION( a ) l_bc += ( a )
#define END() break
#endif
	...
	static void *instructions[] = {
		...
		&&GOTO_asBC_JLowNZ,
		&&GOTO_asBC_AllocMem,
		&&GOTO_asBC_SetListSize,
		&&GOTO_asBC_PshListElmnt,
		&&GOTO_asBC_SetListType,
		&&GOTO_asBC_POWi,
		&&GOTO_asBC_POWu,
		&&GOTO_asBC_POWf,
		&&GOTO_asBC_POWd,
...
// memory access functions
	...
	GOTO( asBC_PshGPtr ):
		...
		*(asPWORD*)l_sp = *(asPWORD*)asBC_PTRARG(l_bc);
		GOTO_INSTRUCTION( 1 + AS_PTR_SIZE );
		END();
...
// path control instructions
	...
	GOTO( asBC_RET ):
		{
			...
			const asWORD w = asBC_WORDARG0(l_bc);
	...
	GOTO( asBC_JMP ):
		GOTO_INSTRUCTION( asBC_INTARG(l_bc) + 2 );
		END();
...
// test instructions
	...
	GOTO( asBC_TZ ):
#if AS_SIZEOF_BOOL == 1
		{
			...
			memset( (void *)regBptr, 0, sizeof( long ) );
...
// bits instructions
	...
	GOTO( asBC_JMPP ):
		GOTO_INSTRUCTION( 1 + (*(int*)(l_fp - asBC_SWORDARG0(l_bc)))*2 );
		END();
	...
	GOTO( asBC_SetG4 ):
		*(asDWORD*)asBC_PTRARG(l_bc) = asBC_DWORDARG(l_bc+AS_PTR_SIZE);
		GOTO_INSTRUCTION( 2 + AS_PTR_SIZE );
		END();
	...
	GOTO( asBC_LoadRObjR ):
		{
			...
			// PopRPtr
			*(asPWORD*)&m_regs.valueRegister = tmp;
			GOTO_INSTRUCTION( asBC_INTARG(l_bc) + 3 );
		}
		END();
	...
	GOTO( 201 ): l_bc = (asDWORD*)201; END();
	GOTO( 202 ): l_bc = (asDWORD*)202; END();
	GOTO( 203 ): l_bc = (asDWORD*)203; END();
	GOTO( 204 ): l_bc = (asDWORD*)204; END();
	GOTO( 205 ): l_bc = (asDWORD*)205; END();
	GOTO( 206 ): l_bc = (asDWORD*)206; END();
	GOTO( 207 ): l_bc = (asDWORD*)207; END();
	GOTO( 208 ): l_bc = (asDWORD*)208; END();
	GOTO( 209 ): l_bc = (asDWORD*)209; END();
	GOTO( 210 ): l_bc = (asDWORD*)210; END();
	GOTO( 211 ): l_bc = (asDWORD*)211; END();
	GOTO( 212 ): l_bc = (asDWORD*)212; END();
	GOTO( 213 ): l_bc = (asDWORD*)213; END();
	GOTO( 214 ): l_bc = (asDWORD*)214; END();
	GOTO( 215 ): l_bc = (asDWORD*)215; END();
	GOTO( 216 ): l_bc = (asDWORD*)216; END();
	GOTO( 217 ): l_bc = (asDWORD*)217; END();
	GOTO( 218 ): l_bc = (asDWORD*)218; END();
	GOTO( 219 ): l_bc = (asDWORD*)219; END();
	GOTO( 220 ): l_bc = (asDWORD*)220; END();
	GOTO( 221 ): l_bc = (asDWORD*)221; END();
	GOTO( 222 ): l_bc = (asDWORD*)222; END();
	GOTO( 223 ): l_bc = (asDWORD*)223; END();
	GOTO( 224 ): l_bc = (asDWORD*)224; END();
	GOTO( 225 ): l_bc = (asDWORD*)225; END();
	GOTO( 226 ): l_bc = (asDWORD*)226; END();
	GOTO( 227 ): l_bc = (asDWORD*)227; END();
	GOTO( 228 ): l_bc = (asDWORD*)228; END();
	GOTO( 229 ): l_bc = (asDWORD*)229; END();
	GOTO( 230 ): l_bc = (asDWORD*)230; END();
	GOTO( 231 ): l_bc = (asDWORD*)231; END();
	GOTO( 232 ): l_bc = (asDWORD*)232; END();
	GOTO( 233 ): l_bc = (asDWORD*)233; END();
	GOTO( 234 ): l_bc = (asDWORD*)234; END();
	GOTO( 235 ): l_bc = (asDWORD*)235; END();
	GOTO( 236 ): l_bc = (asDWORD*)236; END();
	GOTO( 237 ): l_bc = (asDWORD*)237; END();
	GOTO( 238 ): l_bc = (asDWORD*)238; END();
	GOTO( 239 ): l_bc = (asDWORD*)239; END();
	GOTO( 240 ): l_bc = (asDWORD*)240; END();
	GOTO( 241 ): l_bc = (asDWORD*)241; END();
	GOTO( 242 ): l_bc = (asDWORD*)242; END();
	GOTO( 243 ): l_bc = (asDWORD*)243; END();
	GOTO( 244 ): l_bc = (asDWORD*)244; END();
	GOTO( 245 ): l_bc = (asDWORD*)245; END();
	GOTO( 246 ): l_bc = (asDWORD*)246; END();
	GOTO( 247 ): l_bc = (asDWORD*)247; END();
	GOTO( 248 ): l_bc = (asDWORD*)248; END();
	GOTO( 249 ): l_bc = (asDWORD*)249; END();
	GOTO( 250 ): l_bc = (asDWORD*)250; END();
	GOTO( 251 ): l_bc = (asDWORD*)251; END();
	GOTO( 252 ): l_bc = (asDWORD*)252; END();
	GOTO( 253 ): l_bc = (asDWORD*)253; END();
	GOTO( 254 ): l_bc = (asDWORD*)254; END();
	GOTO( 255 ): l_bc = (asDWORD*)255; END();
...
int asCContext::CallGeneric(asCScriptFunction *descr)
{
	...
	// = CreateStackObject( asCGeneric, m_engine, descr, currentObject, args );
