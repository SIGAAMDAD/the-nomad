...
void R_LoadPCX( const char *filename, byte **pic, int *width, int *height, int *channels )
{
	union {
		byte *b;
		void *v;
	...
	*pic = NULL;
	...
	// load the file
	//
	len = ri.FS_LoadFile( filename, &raw.v );
	if (!raw.b || len < 0) {
		return;
	}
	...
	{
		ri.Printf( PRINT_INFO, "PCX truncated: %s\n", filename );
	...
	if (pcx->manufacturer != 0x0a
		...
		|| h >= 1024)
	{
		ri.Printf( PRINT_INFO, "Bad or unsupported pcx file %s (%dx%d@%d)\n", filename, w, h, pcx->bits_per_pixel );
		return;
	}
	...
	if(pix < pic8+size)
	{
		ri.Printf( PRINT_INFO, "PCX file truncated: %s\n", filename );
		ri.FS_FreeFile( pcx );
		ri.Free( pic8 );
	}
	...
	{
		ri.Printf( PRINT_INFO, "PCX missing palette: %s\n", filename );
		ri.FS_FreeFile( pcx );
		ri.Free( pic8 );
		return;
	}
