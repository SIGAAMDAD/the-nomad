...
using UtlUnorderedSet = eastl::unordered_set<Value, eastl::hash<Value>, eastl::equal_to<Value>, CModuleAllocator>;
template<typename T>
using UtlStack = eastl::stack<T, UtlVector<T>>;
...
unsigned Processor::call_thiscall_args(Register* obj, const char* args, va_list ap) {
	UtlStack<Argument> arg_stack;
	UtlUnorderedSet<unsigned char> used_regs;
	Register esp(*this, ESP, sizeof(void*) * 8);
