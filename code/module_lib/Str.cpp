...
void idStr::Copynz( char *dest, const char *src, int destsize ) {
	if ( !src ) {
		Con_Printf( COLOR_YELLOW "idStr::Copynz: NULL src" );
		return;
	}
	if ( destsize < 1 ) {
		Con_Printf( COLOR_YELLOW "idStr::Copynz: destsize < 1" );
		return;
	}
...
void idStr::Append( char *dest, int size, const char *src ) {
	...
	l1 = strlen( dest );
	if ( l1 >= size ) {
		N_Error( ERR_DROP, "idStr::Append: already overflowed" );
	}
...
int idStr::snPrintf( char *dest, int size, const char *fmt, ...) {
	...
	int len;
	va_start( argptr, fmt );
	len = N_vsnprintf(dest, size, fmt, argptr);
	va_end( argptr );
	if ( len >= 32000 ) {
		...
		N_Error( ERR_DROP, "idStr::snPrintf: overflowed buffer" );
	}
	if ( len >= size ) {
		Con_Printf( COLOR_YELLOW "idStr::snPrintf: overflow of %i in %i\n", len, size );
		len = size;
	}
	return len;
}
...
int idStr::vsnPrintf( char *dest, int size, const char *fmt, va_list argptr ) {
	int ret = N_vsnprintf(dest, size, fmt, argptr);
	if ( ret < 0 || ret >= size ) {
		return -1;
	}
	return ret;
