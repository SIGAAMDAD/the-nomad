...
void SDictionaryCache::Setup( asIScriptEngine *engine )
{
	SDictionaryCache *cache = (SDictionaryCache *)engine->GetUserData( DICTIONARY_CACHE );
	if ( cache == 0 ) {
		cache = CreateObject<SDictionaryCache>();
...
void SDictionaryCache::Cleanup( asIScriptEngine *engine )
{
	SDictionaryCache *cache = (SDictionaryCache *)engine->GetUserData( DICTIONARY_CACHE );
	if ( cache ) {
		DeleteObject<SDictionaryCache>( cache );
	}
}
...
void CScriptDictionary::Init(asIScriptEngine *e)
{
	...
	SDictionaryCache *cache = (SDictionaryCache *)m_pEngine->GetUserData( DICTIONARY_CACHE );

	// Notify the garbage collector of this object
	m_pEngine->NotifyGarbageCollectorOfNewObject( this, cache->dictType );
}
	...
	while ( length-- ) {
		...
		else
		{
			if( (typeId & asTYPEID_MASK_OBJECT) &&
				...
				(m_pEngine->GetTypeInfoById(typeId)->GetFlags() & asOBJ_REF) )
			{
		...
		if( typeId & asTYPEID_MASK_OBJECT )
		{
			asITypeInfo *ti = m_pEngine->GetTypeInfoById(typeId);
		...
		{
			buffer += m_pEngine->GetSizeOfPrimitiveType(typeId);
		}
	}
}
...
void CScriptDictionary::AddRef( void ) const
{
...
int CScriptDictionary::GetRefCount( void )
{
	return m_nRefCount;
}

void CScriptDictionary::SetGCFlag( void )
{
	m_bGCFlag = true;
}

bool CScriptDictionary::GetGCFlag( void )
{
	return m_bGCFlag;
}
...
void CScriptDictionary::EnumReferences(asIScriptEngine *inEngine)
{
	...
	for( it = m_Dict.begin(); it != m_Dict.end(); it++ )
	{
		...
		{
			asITypeInfo *subType = m_pEngine->GetTypeInfoById(it->second.m_typeId);
			if ((subType->GetFlags() & asOBJ_VALUE) && (subType->GetFlags() & asOBJ_GC))
			{
				...
				m_pEngine->ForwardGCEnumReferences(it->second.m_valueObj, subType);
			}
			else
			{
...
CScriptDictionary &CScriptDictionary::operator =(const CScriptDictionary &other)
{
	...
	for( it = other.m_Dict.begin(); it != other.m_Dict.end(); it++ )
	{
...
void CScriptDictionary::Set(const dictKey_t &key, void *value, int typeId)
{
	...
	it = m_Dict.find(key);
	if( it == m_Dict.end() )
		it = m_Dict.insert(dictMap_t::value_type(key, CScriptDictValue())).first;

	it->second.Set(m_pEngine, value, typeId);
}

void CScriptDictionary::Set(const dictKey_t &key, const asINT32& value)
{
	Set(key, const_cast<asINT32 *>( &value ), asTYPEID_INT32);
}

void CScriptDictionary::Set(const dictKey_t &key, const asDWORD& value)
{
	Set(key, const_cast<asDWORD *>( &value ), asTYPEID_UINT32);
}
...
bool CScriptDictionary::Get(const dictKey_t &key, void *value, int typeId) const
{
	...
	it = m_Dict.find(key);
	if( it != m_Dict.end() )
		return it->second.Get(m_pEngine, value, typeId);
...
bool CScriptDictionary::Get(const dictKey_t &key, asINT32& value) const
{
	return Get( key, &value, asTYPEID_INT32 );
}

bool CScriptDictionary::Get(const dictKey_t &key, asDWORD& value) const
{
	return Get( key, &value, asTYPEID_UINT32 );
}
...
CScriptArray* CScriptDictionary::GetKeys() const
{
	...
	CScriptArray *array = CScriptArray::Create(ti, asUINT(m_Dict.size()));
	long current = -1;
	dictMap_t::const_iterator it;
	for( it = m_Dict.begin(); it != m_Dict.end(); it++ )
	{
...
void ScriptDictionarySetInt32_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
	dict->Set(*key, *(asINT32 *)ref);
}

void ScriptDictionarySetUInt32_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
	dict->Set(*key, *(asDWORD *)ref);
}

void ScriptDictionarySetInt64_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
...
void ScriptDictionarySetUInt64_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
...
void ScriptDictionaryGetInt32_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
	*(bool*)gen->GetAddressOfReturnLocation() = dict->Get(*key, *(asINT32 *)ref);
}

void ScriptDictionaryGetUInt32_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
	*(bool*)gen->GetAddressOfReturnLocation() = dict->Get(*key, *(asDWORD *)ref);
}

void ScriptDictionaryGetInt64_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
...
void ScriptDictionaryGetUInt64_Generic(asIScriptGeneric *gen)
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObjectData();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	void *ref = *(void**)gen->GetAddressOfArg(1);
	*(bool*)gen->GetAddressOfReturnLocation() = dict->Get(*key, *(asQWORD *)ref);
}
...
void CScriptDictValue::Set(asIScriptEngine *engine, void *value, int typeId)
{
	...
	else
	{
		...
		switch ( typeId ) {
		case asTYPEID_FLOAT:
			m_valueFlt = *(float *)value;
			break;
		case asTYPEID_DOUBLE:
			m_valueFlt = *(double *)value;
			break;
		case asTYPEID_UINT8:
			m_valueInt = *(uint8_t *)value;
			break;
		case asTYPEID_UINT16:
			m_valueInt = *(uint16_t *)value;
			break;
		case asTYPEID_UINT32:
			m_valueInt = *(uint32_t *)value;
			break;
		case asTYPEID_UINT64:
			m_valueInt = *(uint64_t *)value;
			break;
		case asTYPEID_INT8:
			m_valueInt = *(int8_t *)value;
			break;
		case asTYPEID_INT16:
			m_valueInt = *(int16_t *)value;
			break;
		case asTYPEID_INT32:
			m_valueInt = *(int32_t *)value;
			break;
		case asTYPEID_INT64:
			m_valueInt = *(int64_t *)value;
			break;
		default:
...
void CScriptDictValue::Set(asIScriptEngine *engine, const asINT32& value)
{
	Set(engine, const_cast<asINT32 *>( &value ), asTYPEID_INT32);
}

void CScriptDictValue::Set(asIScriptEngine *engine, const asDWORD& value)
{
	Set(engine, const_cast<asDWORD *>( &value ), asTYPEID_UINT32);
}
...
bool CScriptDictValue::Get(asIScriptEngine *engine, asINT32& value) const
{
	return Get(engine, &value, asTYPEID_INT32);
}

bool CScriptDictValue::Get(asIScriptEngine *engine, asDWORD& value) const
{
	return Get(engine, &value, asTYPEID_UINT32);
}
...
static CScriptDictValue &CScriptDictValue_opAssign(asINT32 val, CScriptDictValue *obj)
{
	return CScriptDictValue_opAssign(&val, asTYPEID_INT32, obj);
}

static CScriptDictValue &CScriptDictValue_opAssign(asDWORD val, CScriptDictValue *obj)
{
	return CScriptDictValue_opAssign(&val, asTYPEID_UINT32, obj);
}
...
static CScriptDictValue &CScriptDictValue_opAssign(asQWORD val, CScriptDictValue *obj)
{
...
static asINT32 CScriptDictValue_opConvInt32(CScriptDictValue *obj)
{
	asINT32 value;
	CScriptDictValue_opCast(&value, asTYPEID_INT32, obj);
	return value;
}

static asDWORD CScriptDictValue_opConvUInt32(CScriptDictValue *obj)
{
	asDWORD value;
	CScriptDictValue_opCast(&value, asTYPEID_UINT32, obj);
	return value;
}

static asINT64 CScriptDictValue_opConvInt64(CScriptDictValue *obj)
{
	asINT64 value;
...
static asQWORD CScriptDictValue_opConvUInt64(CScriptDictValue *obj)
{
	asQWORD value;
...
static void CScriptDictValue_opConvInt32_Generic(asIScriptGeneric *gen)
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	asINT32 value;
	self->Get(gen->GetEngine(), value);
	*(asINT32 *)gen->GetAddressOfReturnLocation() = value;
}

static void CScriptDictValue_opConvUInt32_Generic(asIScriptGeneric *gen)
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	asDWORD value;
	self->Get(gen->GetEngine(), value);
	*(asDWORD *)gen->GetAddressOfReturnLocation() = value;
}

static void CScriptDictValue_opConvInt64_Generic(asIScriptGeneric *gen)
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	asINT64 value;
	self->Get(gen->GetEngine(), value);
...
static void CScriptDictValue_opConvUInt64_Generic(asIScriptGeneric *gen)
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	asQWORD value;
	self->Get(gen->GetEngine(), value);
...
static void CScriptDictValue_opAssign_int32_Generic(asIScriptGeneric *gen)
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	*(CScriptDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign( gen->GetArgDWord(0), self );
}

static void CScriptDictValue_opAssign_uint32_Generic(asIScriptGeneric *gen)
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	*(CScriptDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign( gen->GetArgDWord(0), self );
}
...
{
	CScriptDictValue *self = (CScriptDictValue*)gen->GetObjectData();
	*(CScriptDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign( gen->GetArgQWord(0), self );
}
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	CheckASCall( engine->RegisterObjectType("dictionaryValue", sizeof(CScriptDictValue), asOBJ_VALUE | asOBJ_ASHANDLE | asOBJ_GC | asGetTypeTraits<CScriptDictValue>()) );
#else
	CheckASCall( engine->RegisterObjectType("dictionaryValue", sizeof(CScriptDictValue), asOBJ_VALUE | asOBJ_ASHANDLE | asOBJ_GC | asOBJ_APP_CLASS_CD) );
#endif
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_CONSTRUCT, "void f()", asFUNCTION(CScriptDictValue_Construct), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_DESTRUCT, "void f()", asFUNCTION(CScriptDictValue_Destruct), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(CScriptDictValue, EnumReferences), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(CScriptDictValue, FreeValue), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const dictionaryValue &in)", asFUNCTIONPR(CScriptDictValue_opAssign, (const CScriptDictValue &, CScriptDictValue *), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opHndlAssign(const ?&in)", asFUNCTIONPR(CScriptDictValue_opAssign, (void *, int, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opHndlAssign(const dictionaryValue &in)", asFUNCTIONPR(CScriptDictValue_opAssign, (const CScriptDictValue &, CScriptDictValue *), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const ?&in)", asFUNCTIONPR(CScriptDictValue_opAssign, (void *, int, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(double)", asFUNCTIONPR(CScriptDictValue_opAssign, (double, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(int32)", asFUNCTIONPR(CScriptDictValue_opAssign, (asINT32, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(int32)", asFUNCTIONPR(CScriptDictValue_opAssign, (asDWORD, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opCast(?&out)", asFUNCTIONPR(CScriptDictValue_opCast, (void *, int, CScriptDictValue*), void), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opConv(?&out)", asFUNCTIONPR(CScriptDictValue_opCast, (void *, int, CScriptDictValue*), void), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "int32 opConv()", asFUNCTIONPR(CScriptDictValue_opConvInt32, (CScriptDictValue*), asINT32), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "uint32 opConv()", asFUNCTIONPR(CScriptDictValue_opConvUInt32, (CScriptDictValue*), asDWORD), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "double opConv()", asFUNCTIONPR(CScriptDictValue_opConvDouble, (CScriptDictValue*), double), asCALL_CDECL_OBJLAST) );

	CheckASCall( engine->RegisterObjectType("dictionary", sizeof(CScriptDictionary), asOBJ_REF | asOBJ_GC) );
	...
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_FACTORY, "dictionary@ f()", asFUNCTION(ScriptDictionaryFactory_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_LIST_FACTORY, "dictionary @f(int &in) {repeat {string, ?}}", asFUNCTION(ScriptDictionaryListFactory_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()", asMETHOD(CScriptDictionary,AddRef), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASE, "void f()", asMETHOD(CScriptDictionary,Release), asCALL_THISCALL) );
	...
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int32& in)", asMETHODPR(CScriptDictionary,Set,(const dictKey_t&,const asINT32&),void), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const uint32& in)", asMETHODPR(CScriptDictionary,Set,(const dictKey_t&,const asDWORD&),void), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int32& out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,asINT32&) const,bool), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, uint32& out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,asDWORD&) const,bool), asCALL_THISCALL) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const double& in)", asMETHODPR(CScriptDictionary,Set,(const dictKey_t&,const double&),void), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, double& out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,double&) const,bool), asCALL_THISCALL) );
	...
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, int32& out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,asINT32&) const,bool), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, uint32& out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,asDWORD&) const,bool), asCALL_THISCALL) );	
	...
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(CScriptDictionary,GetRefCount), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(CScriptDictionary,SetGCFlag), asCALL_THISCALL) );
...
void RegisterScriptDictionary_Generic(asIScriptEngine *engine)
{
	...
	CheckASCall( engine->RegisterObjectType("dictionaryValue", sizeof(CScriptDictValue), asOBJ_VALUE | asOBJ_GC | asOBJ_ASHANDLE | asGetTypeTraits<CScriptDictionary>()) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_CONSTRUCT, "void f()", asFUNCTION(CScriptDictValue_Construct_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_DESTRUCT, "void f()", asFUNCTION(CScriptDictValue_Destruct_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_ENUMREFS, "void f(int&in)", asFUNCTION(CScriptDictValue_EnumReferences_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_RELEASEREFS, "void f(int&in)", asFUNCTION(CScriptDictValue_FreeValue_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const dictionaryValue &in)", asFUNCTION(CScriptDictValue_opCopyAssign_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opHndlAssign(const ?&in)", asFUNCTION(CScriptDictValue_opAssign_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opHndlAssign(const dictionaryValue &in)", asFUNCTION(CScriptDictValue_opCopyAssign_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const ?&in)", asFUNCTION(CScriptDictValue_opAssign_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(double)", asFUNCTION(CScriptDictValue_opAssign_double_Generic), asCALL_GENERIC) );
//	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(int)", asFUNCTION(CScriptDictValue_opAssign_int32_Generic), asCALL_GENERIC) );
//	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(uint)", asFUNCTION(CScriptDictValue_opAssign_uint32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(int64)", asFUNCTION(CScriptDictValue_opAssign_int64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(uint64)", asFUNCTION(CScriptDictValue_opAssign_uint64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opCast(?&out)", asFUNCTION(CScriptDictValue_opCast_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opConv(?&out)", asFUNCTION(CScriptDictValue_opCast_Generic), asCALL_GENERIC) );
//	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "int opConv()", asFUNCTION(CScriptDictValue_opConvInt32_Generic), asCALL_GENERIC) );
//	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "uint opConv()", asFUNCTION(CScriptDictValue_opConvUInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "int64 opConv()", asFUNCTION(CScriptDictValue_opConvInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "uint64 opConv()", asFUNCTION(CScriptDictValue_opConvUInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "double opConv()", asFUNCTION(CScriptDictValue_opConvDouble_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectType("dictionary", sizeof(CScriptDictionary), asOBJ_REF | asOBJ_GC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_FACTORY, "dictionary@ f()", asFUNCTION(ScriptDictionaryFactory_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_LIST_FACTORY, "dictionary @f(int &in) {repeat {string, ?}}", asFUNCTION(ScriptDictionaryListFactory_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()", asFUNCTION(ScriptDictionaryAddRef_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASE, "void f()", asFUNCTION(ScriptDictionaryRelease_Generic), asCALL_GENERIC) );
	...
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool Contains(const string& in) const", asFUNCTION(ScriptDictionaryExists_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool Remove(const string& in)", asFUNCTION(ScriptDictionaryDelete_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void Clear()", asFUNCTION(ScriptDictionaryDeleteAll_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, ?&out) const", asFUNCTION(ScriptDictionaryGet_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, int32& out) const", asFUNCTION(ScriptDictionaryGetInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, uint32& out) const", asFUNCTION(ScriptDictionaryGetUInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, int64& out) const", asFUNCTION(ScriptDictionaryGetInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, uint64& out) const", asFUNCTION(ScriptDictionaryGetUInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, double& out) const", asFUNCTION(ScriptDictionaryGetFlt_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const ?&in)", asFUNCTION(ScriptDictionarySet_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, ?&out) const", asFUNCTION(ScriptDictionaryGet_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int& in)", asFUNCTION(ScriptDictionarySetInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const uint& in)", asFUNCTION(ScriptDictionarySetUInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int& out) const", asFUNCTION(ScriptDictionaryGetInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, uint& out) const", asFUNCTION(ScriptDictionaryGetUInt32_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int64& in)", asFUNCTION(ScriptDictionarySetInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const uint64& in)", asFUNCTION(ScriptDictionarySetUInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int64& out) const", asFUNCTION(ScriptDictionaryGetInt64_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, uint64& out) const", asFUNCTION(ScriptDictionaryGetUInt64_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const double&in)", asFUNCTION(ScriptDictionarySetFlt_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, double&out) const", asFUNCTION(ScriptDictionaryGetFlt_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "dictionaryValue &opIndex(const string &in)", asFUNCTION(CScriptDictionary_opIndex_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "const dictionaryValue &opIndex(const string &in) const", asFUNCTION(CScriptDictionary_opIndex_const_Generic), asCALL_GENERIC) );

	// Register GC behaviours
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETREFCOUNT, "int f()", asFUNCTION(ScriptDictionaryGetRefCount_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_SETGCFLAG, "void f()", asFUNCTION(ScriptDictionarySetGCFlag_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETGCFLAG, "bool f()", asFUNCTION(ScriptDictionaryGetGCFlag_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ENUMREFS, "void f(int&in)", asFUNCTION(ScriptDictionaryEnumReferences_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASEREFS, "void f(int&in)", asFUNCTION(ScriptDictionaryReleaseAllReferences_Generic), asCALL_GENERIC) );
...
bool CScriptDictionary::CIterator::GetValue(asINT32& value) const
{
	return m_it->second.Get(m_Dict.m_pEngine, &value, asTYPEID_INT32 );
}

bool CScriptDictionary::CIterator::GetValue(asDWORD& value) const
{
	return m_it->second.Get(m_Dict.m_pEngine, &value, asTYPEID_UINT32 );
}

bool CScriptDictionary::CIterator::GetValue(asINT64& value) const
{
	return m_it->second.Get(m_Dict.m_pEngine, &value, asTYPEID_INT64 );
}

bool CScriptDictionary::CIterator::GetValue(asQWORD& value) const
{
	return m_it->second.Get(m_Dict.m_pEngine, &value, asTYPEID_UINT64 );
}


bool CScriptDictionary::CIterator::GetValue(double &value) const
{
	return m_it->second.Get(m_Dict.m_pEngine, &value, asTYPEID_DOUBLE);
}

bool CScriptDictionary::CIterator::GetValue(void *value, int typeId) const
{
	return m_it->second.Get(m_Dict.m_pEngine, value, typeId);
}
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const ?&in)", asFUNCTIONPR(CScriptDictValue_opAssign, (void *, int, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(double)", asFUNCTIONPR(CScriptDictValue_opAssign, (double, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(int64)", asFUNCTIONPR(CScriptDictValue_opAssign, (asINT64, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opCast(?&out)", asFUNCTIONPR(CScriptDictValue_opCast, (void *, int, CScriptDictValue*), void), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opConv(?&out)", asFUNCTIONPR(CScriptDictValue_opCast, (void *, int, CScriptDictValue*), void), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "int64 opConv()", asFUNCTIONPR(CScriptDictValue_opConvInt, (CScriptDictValue*), asINT64), asCALL_CDECL_OBJLAST) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "double opConv()", asFUNCTIONPR(CScriptDictValue_opConvDouble, (CScriptDictValue*), double), asCALL_CDECL_OBJLAST) );
	...
	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int64&in)", asMETHODPR(CScriptDictionary,Set,(const dictKey_t&,const asINT64&),void), asCALL_THISCALL) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int64&out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,asINT64&) const,bool), asCALL_THISCALL) );
...
void RegisterScriptDictionary_Generic(asIScriptEngine *engine)
{
	...
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opCast(?&out)", asFUNCTION(CScriptDictValue_opCast_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "void opConv(?&out)", asFUNCTION(CScriptDictValue_opCast_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "int64 opConv()", asFUNCTION(CScriptDictValue_opConvInt_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionaryValue", "double opConv()", asFUNCTION(CScriptDictValue_opConvDouble_Generic), asCALL_GENERIC) );
	...
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, ?&out) const", asFUNCTION(ScriptDictionaryGet_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, int64&out) const", asFUNCTION(ScriptDictionaryGetInt_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool TryGetValue(const string &in, double&out) const", asFUNCTION(ScriptDictionaryGetFlt_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const ?&in)", asFUNCTION(ScriptDictionarySet_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, ?&out) const", asFUNCTION(ScriptDictionaryGet_Generic), asCALL_GENERIC) );

	CheckASCall( engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int64&in)", asFUNCTION(ScriptDictionarySetInt_Generic), asCALL_GENERIC) );
	CheckASCall( engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int64&out) const", asFUNCTION(ScriptDictionaryGetInt_Generic), asCALL_GENERIC) );
