...
#include "code/sgame/sg_public.h"
...
static qboolean find_free_rx( void ) {
	uint32_t i, n;
#if 1
	for ( i = 0; i < arraylen( rx_list_alloc ); i++ ) {
		n = rx_list_alloc[i];
...
static void wipe_reg_range( reg_t *reg, const var_addr_t *v ) {
	if ( reg->type_mask & RTYPE_VAR ) {
		uint32_t c, n;
		for ( c = 0, n = 0; n < arraylen( reg->vars.map ); n++ ) {
...
static void wipe_var_range( const var_addr_t *v )
{
	...
	for ( i = 0; i < arraylen( rx_regs ); i++ ) {
		wipe_reg_range( &rx_regs[i], v );
	}
	for ( i = 0; i < arraylen( sx_regs ); i++ ) {
...
static void set_var_map( reg_t *r, const var_addr_t *v ) {
	uint32_t n;
	for ( n = 0; n < arraylen( r->vars.map ); n++ ) {
		if ( r->vars.map[n].size == 0 ) {
			r->vars.map[n] = *v;
			r->vars.idx = ( n + 1 ) % arraylen( r->vars.map );
			return;
		}
	}
	r->vars.map[r->vars.idx] = *v;
	r->vars.idx = ( r->vars.idx + 1 ) % arraylen( r->vars.map );
}


static void set_rx_var( uint32_t reg, const var_addr_t *v ) {
#ifdef LOAD_OPTIMIZE
	if ( reg < arraylen( rx_regs ) ) {
		reg_t *r = rx_regs + reg;
#ifdef REG_TYPE_MASK
...
static void reduce_map_size( reg_t *reg, uint32_t size ) {
	int i;
	for ( i = 0; i < arraylen( reg->vars.map ); i++ ) {
...
static void init_opstack( void )
{
	...
	memset( &rx_mask[0], 0, sizeof( rx_mask ) );
	memset( &sx_mask[0], 0, sizeof( sx_mask ) );

	memset( &opstackv[0], 0, sizeof( opstackv ) );

	memset( &rx_regs[0], 0, sizeof( rx_regs ) );
	memset( &sx_regs[0], 0, sizeof( sx_regs ) );
}
...
static reg_t *find_rx_const( uint32_t imm, uint32_t mask )
{
	...
	for ( i = 0; i < arraylen( rx_list_cache ); i++ ) {
		n = rx_list_cache[ i ];
		r = &rx_regs[ n ];
...
static uint32_t alloc_sx_const( uint32_t pref, uint32_t imm )
{
	...
	if ( ( pref & FORCED ) == 0 ) {
		...
		for ( i = 0; i < arraylen( sx_list_cache ); i++ ) {
			n = sx_list_cache[i];
			if ( mask & ( 1 << n ) ) {
				// target register must be unmasked and not present on the opStack
				continue;
...
static uint32_t dyn_alloc_sx( uint32_t pref )
{
	...
	// try to bypass registers with metadata
	for ( i = 0; i < arraylen( sx_list_alloc ); i++ ) {
		n = sx_list_alloc[i];
		if ( mask & ( 1 << n ) ) {
			continue;
		}
...
static const GDR_INLINE qboolean HasSSEFP( void )
{
#if GDRi386
...
{
	if ( vm->forceDataMask || !( vm_rtChecks->i & VM_RTCHECK_DATA ) )
	{
#if GDRx64
...
{
	if ( ( vm_rtChecks->i & VM_RTCHECK_JUMP ) == 0 ) {
		return;
	}
...
static void emit_CheckProc( vm_t *vm, instruction_t *ins )
{
	...
	if ( vm_rtChecks->i & VM_RTCHECK_PSTACK ) {
#if GDRx64
	...
	if ( vm_rtChecks->i & VM_RTCHECK_OPSTACK ) {
...
static void EmitBCPYFunc( vm_t *vm )
{
	...
	if ( vm_rtChecks->i & VM_RTCHECK_DATA )
	{
...
#if GDRi386
	vm->opStackTop = opStack + arraylen( opStack ) - 1;
#endif
