...
static void mdfour_tail(const byte *in, uint64_t n)
{
	byte buf[128];
	uint32_t M[16];
	uint32_t b;
...
static void mdfour_update(struct mdfour *md, const byte *in, uint64_t n)
{
	uint32_t M[16];
...
static void mdfour(byte *out, const byte *in, uint64_t n)
{
	struct mdfour md;
	mdfour_begin(&md);
