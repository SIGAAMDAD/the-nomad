...
inline eastl::string& get_base64_chars() {
    static eastl::string base64_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                   "abcdefghijklmnopqrstuvwxyz"
                                   "0123456789+/";
    return base64_chars;
}

inline std::string compress(std::string const &data)
{
  int counter = 0;
  uint32_t bit_stream = 0;
  const eastl::string& base64_chars = get_base64_chars();
  std::string encoded;
  int offset = 0;
...
inline std::string uncompress(std::string const &data)
{
  int counter = 0;
  uint32_t bit_stream = 0;
  std::string decoded;
