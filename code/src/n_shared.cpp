...
static const char *Com_StringContains( const char *str1, const char *str2, uint64_t len2 )
{
	int64_t len, i, j;
...
{
	if (!dest)
		Com_Error("N_strncpyz: NULL dest");
	if (!src)
		Com_Error("N_strncpyz: NULL src");
	if (count < 1)
		Com_Error("N_strncpyz: bad count");
...
void N_strcat(char *dest, size_t size, const char *src)
{
	...
	l1 = strlen(dest);
	if (l1 >= size)
		Com_Error( "N_strcat: already overflowed" );
