...
GDR_EXPORT void R_GLDebug_Callback_AMD(GLuint id, GLenum category, GLenum severity, GLsizei length, const GLchar *message, GLvoid *userParam);
// the normal stuff
GDR_EXPORT void R_GLDebug_Callback_ARB(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const GLvoid *userParam);
...
================
*/
GDR_EXPORT void R_PrintLongString(const char *string)
{
	char buffer[8192];
	const char *p;
...
GDR_EXPORT void GpuMemInfo_f(void)
{
...
GDR_EXPORT byte *RB_ReadPixels(uint32_t x, uint32_t y, uint32_t width, uint32_t height, size_t *offset, uint32_t *padlen)
{
	byte *buffer, *bufstart;
	uint32_t padwidth, linelen;
	GLint packAlign;
...
================== 
*/
GDR_EXPORT void R_ScreenshotFilename( uint32_t lastNumber, char *fileName )
{
	uint32_t a,b,c,d;
...
====================
*/
GDR_EXPORT void R_LevelShot( void )
{
	...
	Com_snprintf(checkname, sizeof(checkname), "levelshots/%s.tga", rg.world->GetBaseName());
...
==================
*/
GDR_EXPORT void R_TakeScreenshot( uint32_t x, uint32_t y, uint32_t width, uint32_t height, char *name, qboolean jpeg ) {
...
================== 
*/
GDR_EXPORT void R_ScreenShot_f(void)
{
    char checkname[MAX_OSPATH];
    static uint32_t lastNumber = -1;
    qboolean silent;
...
GDR_EXPORT void GpuInfo_f( void ) 
{
...
GDR_EXPORT void R_Register(void)
{
    //
...
GDR_EXPORT void R_InitGLContext(void)
{
...
GDR_EXPORT void R_InitImGui(void)
{
    ...
    ri.ImGui_Init((void *)(uintptr_t)rg.imguiShader.GetProgram(), &import);
}

GDR_EXPORT void R_Init(void)
{
    GLenum error;
    ...
    backendData = (renderBackendData_t *)ri.Hunk_Alloc(sizeof(*backendData) + sizeof(srfPoly_t) * r_maxPolys->i + sizeof(polyVert_t) * r_maxPolyVerts->i, h_low);
...
GDR_EXPORT void RE_Shutdown(refShutdownCode_t code)
{
...
#define MAX_CACHED_GL_MESSAGES 528
static eastl::fixed_vector<char *, MAX_CACHED_GL_MESSAGES> cachedGLMessages;
static uint32_t numCachedGLMessages = 0;
...
GDR_EXPORT void R_GLDebug_Callback_ARB(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const GLvoid *userParam)
{
    ...
    newMsg = (char *)ri.Hunk_Alloc( len, h_low );
    newMsg[len - 1] = '\0';
    strcpy(newMsg, message);

    cachedGLMessages.emplace_back( newMsg );
    numCachedGLMessages++;
