...
GDR_EXPORT void R_PerformanceCounters(void)
{
	if (!r_speeds->i) {
...
GDR_EXPORT void R_IssueRenderCommands(qboolean runPerformanceCounters)
{
...
============
*/
GDR_EXPORT void *R_GetCommandBufferReserved( uint32_t bytes, uint32_t reservedBytes )
{
	renderCommandList_t	*cmdList;
...
=============
*/
GDR_EXPORT void *R_GetCommandBuffer( uint32_t bytes ) {
	...
	nhandle_t hShader )
{
	...
	cmd = (drawImageCmd_t *)R_GetCommandBuffer(sizeof(*cmd));
	if (!cmd) {
		return;
	}
...
//    }
    ...
    if (glConfig.stereoEnabled) {
		if (!(cmd = (drawBufferCmd_t *)R_GetCommandBuffer(sizeof(*cmd))))
			return;
...
GDR_EXPORT void RE_EndFrame(uint64_t *frontEndMsec, uint64_t *backEndMsec)
{
	...
	cmd = (swapBuffersCmd_t *)R_GetCommandBufferReserved(sizeof(*cmd), 0);
	if (!cmd) {
		return;
	}
