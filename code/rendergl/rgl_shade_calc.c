...
static float *TableForFunc( genFunc_t func ) 
{
	...
	case GF_SIN:
		return rg.sinTable;
	case GF_TRIANGLE:
		return rg.triangleTable;
	case GF_SQUARE:
		return rg.squareTable;
	case GF_SAWTOOTH:
		return rg.sawToothTable;
	case GF_INVERSE_SAWTOOTH:
		return rg.inverseSawToothTable;
	case GF_NONE:
	default:
		break;
	}

	ri.Error( ERR_DROP, "TableForFunc called with invalid function '%d' in shader '%s'", func, backend.drawBatch.shader->name );
	return NULL;
}
...
//	int16_t	*normal = tess.normal[0];
	...
	if ( ds->deformationWave.frequency == 0 )
	{
		...
		{
//			R_VaoUnpackNormal(offset, vtx[i].normal);
...
static void RB_CalcDeformNormals( deformStage_t *ds ) {
	...
	for ( i = 0; i < backend.drawBatch.vtxOffset; i++, xyz += 4, normal += 4 ) {
		vec3_t fNormal;
...
static void RB_CalcMoveVertexes( deformStage_t *ds ) {
    ...
    vtx = (polyVert_t *)backend.drawBatch.vertices;
	for ( i = 0; i < backend.drawBatch.vtxOffset; i++, vtx++ ) {
		VectorAdd( vtx->xyz, offset, vtx->xyz );
	}
}
...
static void AutospriteDeform( void ) {
	...
	oldVerts = backend.drawBatch.vtxOffset;
	backend.drawBatch.vtxOffset = 0;
	tess.numIndexes = 0;
	tess.firstIndex = 0;
...
//		VectorCopy( backend.viewParms.or.axis[0], forward );
	...
	for ( i = 0, indexes = 0 ; i < backend.drawBatch.vtxOffset ; i+=4, indexes+=6 ) {
		float	lengths[2];
		int		nums[2];
		vec3_t	mid[2];
		vec3_t	major, minor;
...
float RB_CalcWaveColorSingle( const waveForm_t *wf )
{
	...
	if ( wf->func == GF_NOISE ) {
		...
		glow = EvalWaveForm( wf ) * rg.identityLight;
	}
...
void RB_CalcModulateColorsByFog( unsigned char *colors ) {
	...
	for ( i = 0; i < backend.drawBatch.vtxOffset; i++, colors += 4 ) {
...
void RB_CalcFogTexCoords( float *st ) {
	...
	for (i = 0, v = tess.xyz[0] ; i < backend.drawBatch.vtxOffset ; i++, v += 4) {
...
{
	double timeScale = backend.refdef.floatTime;
...
{
	double timeScale = backend.refdef.floatTime; // -EC- set to double
	...
	sinValue = rg.sinTable[ index & FUNCTABLE_MASK ];
	cosValue = rg.sinTable[ ( index + FUNCTABLE_SIZE / 4 ) & FUNCTABLE_MASK ];
