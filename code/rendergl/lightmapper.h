...
static void lm_integrateHemisphereBatch(lm_context *ctx)
{
	...
	nglBindVertexArray(ctx->hemisphere.vao);
	...
	nglBindFramebuffer(GL_FRAMEBUFFER, ctx->hemisphere.fb[fbWrite]);
	nglFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ctx->hemisphere.fbTexture[fbWrite], 0);
	nglViewport(0, 0, outHemiSize * ctx->hemisphere.fbHemiCountX, outHemiSize * ctx->hemisphere.fbHemiCountY);
	nglUseProgram(ctx->hemisphere.firstPass.programID);
	nglUniform1i(ctx->hemisphere.firstPass.hemispheresTextureID, 0);
	nglActiveTexture(GL_TEXTURE0);
	nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.fbTexture[fbRead]);
	nglUniform1i(ctx->hemisphere.firstPass.weightsTextureID, 1);
	nglActiveTexture(GL_TEXTURE1);
	nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.firstPass.weightsTexture);
	nglActiveTexture(GL_TEXTURE0);
	...
	// downsampling passes
	nglUseProgram(ctx->hemisphere.downsamplePass.programID);
	nglUniform1i(ctx->hemisphere.downsamplePass.hemispheresTextureID, 0);
	while (outHemiSize > 1)
	{
	...
	nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.storage.texture);
	nglCopyTexSubImage2D(GL_TEXTURE_2D, 0,
...
static void lm_writeResultsToLightmap(lm_context *ctx)
{
	...
	nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.storage.texture);
	nglGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_FLOAT, hemi);
...
static lm_bool lm_beginSampleHemisphere(lm_context *ctx, int* viewport, float* view, float* proj)
{
	...
	{
		// prepare hemisphere
		nglBindFramebuffer(GL_FRAMEBUFFER, ctx->hemisphere.fb[0]);
		if (ctx->hemisphere.fbHemiIndex == 0)
		{
			// prepare hemisphere batch
			nglClearColor( // clear to valid background pixels!
...
{
	GLuint shader = nglCreateShader(type);
	if (shader == 0)
	{
		fprintf(stderr, "Could not create shader!\n");
		return 0;
	}
	nglShaderSource(shader, 1, &source, NULL);
	...
	if (!compiled)
	{
		...
		GLint infoLen = 0;
		nglGetShaderiv(shader, GL_INFO_LOG_LENGTH, &infoLen);
		if (infoLen)
		{
			char* infoLog = (char*)malloc(infoLen);
			nglGetShaderInfoLog(shader, infoLen, NULL, infoLog);
			fprintf(stderr, "%s\n", infoLog);
			free(infoLog);
		}
...
{
	GLuint program = nglCreateProgram();
	if (program == 0)
	{
	...
	nglAttachShader(program, vertexShader);
	nglAttachShader(program, fragmentShader);
	nglLinkProgram(program);
	nglDeleteShader(vertexShader);
	nglDeleteShader(fragmentShader);
	GLint linked;
	...
	if (!linked)
	{
		...
		GLint infoLen = 0;
		nglGetProgramiv(program, GL_INFO_LOG_LENGTH, &infoLen);
		if (infoLen)
		{
			char* infoLog = (char*)malloc(sizeof(char) * infoLen);
			nglGetProgramInfoLog(program, infoLen, NULL, infoLog);
			fprintf(stderr, "%s\n", infoLog);
			free(infoLog);
		}
	...
	float cameraToSurfaceDistanceModifier)
{
	...
	nglGenTextures(2, ctx->hemisphere.fbTexture);
	nglGenFramebuffers(2, ctx->hemisphere.fb);
	nglGenRenderbuffers(1, &ctx->hemisphere.fbDepth);

	nglBindRenderbuffer(GL_RENDERBUFFER, ctx->hemisphere.fbDepth);
	nglRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, w[0], h[0]);
	nglBindFramebuffer(GL_FRAMEBUFFER, ctx->hemisphere.fb[0]);
	nglFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, ctx->hemisphere.fbDepth);
	for (int i = 0; i < 2; i++)
	{
		nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.fbTexture[i]);
		...
		nglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, w[i], h[i], 0, GL_RGBA, GL_FLOAT, 0);

		nglBindFramebuffer(GL_FRAMEBUFFER, ctx->hemisphere.fb[i]);
		nglFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ctx->hemisphere.fbTexture[i], 0);
		GLenum status = nglCheckFramebufferStatus(GL_FRAMEBUFFER);
		if (status != GL_FRAMEBUFFER_COMPLETE)
		{
			fprintf(stderr, "Could not create framebuffer!\n");
			nglDeleteRenderbuffers(1, &ctx->hemisphere.fbDepth);
			nglDeleteFramebuffers(2, ctx->hemisphere.fb);
			nglDeleteTextures(2, ctx->hemisphere.fbTexture);
			LM_FREE(ctx);
			return NULL;
		}
	}
	...
	nglGenVertexArrays(1, &ctx->hemisphere.vao);
	...
	// hemisphere shader (weighted downsampling of the 3x1 hemisphere layout to a 0.5x0.5 square)
	{
		...
		if (!ctx->hemisphere.firstPass.programID)
		{
			...
			nglDeleteVertexArrays(1, &ctx->hemisphere.vao);
			nglDeleteRenderbuffers(1, &ctx->hemisphere.fbDepth);
			nglDeleteFramebuffers(2, ctx->hemisphere.fb);
			nglDeleteTextures(2, ctx->hemisphere.fbTexture);
			LM_FREE(ctx);
		...
		ctx->hemisphere.firstPass.hemispheresTextureID = nglGetUniformLocation(ctx->hemisphere.firstPass.programID, "hemispheres");
		ctx->hemisphere.firstPass.weightsTextureID = nglGetUniformLocation(ctx->hemisphere.firstPass.programID, "weights");
	}
	...
	// downsample shader
	{
		...
		if (!ctx->hemisphere.downsamplePass.programID)
		{
			...
			nglDeleteProgram(ctx->hemisphere.firstPass.programID);
			nglDeleteVertexArrays(1, &ctx->hemisphere.vao);
			nglDeleteRenderbuffers(1, &ctx->hemisphere.fbDepth);
			nglDeleteFramebuffers(2, ctx->hemisphere.fb);
			nglDeleteTextures(2, ctx->hemisphere.fbTexture);
		...
		ctx->hemisphere.downsamplePass.hemispheresTextureID = nglGetUniformLocation(ctx->hemisphere.downsamplePass.programID, "hemispheres");
	}

	// hemisphere weights texture
	nglGenTextures(1, &ctx->hemisphere.firstPass.weightsTexture);
	lmSetHemisphereWeights(ctx, lm_defaultWeights, 0);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
void lmDestroy(lm_context *ctx)
{
	...
	// delete gl objects
	nglDeleteTextures(1, &ctx->hemisphere.firstPass.weightsTexture);
	nglDeleteTextures(1, &ctx->hemisphere.storage.texture);
	nglDeleteProgram(ctx->hemisphere.downsamplePass.programID);
	nglDeleteProgram(ctx->hemisphere.firstPass.programID);
	nglDeleteVertexArrays(1, &ctx->hemisphere.vao);
	nglDeleteRenderbuffers(1, &ctx->hemisphere.fbDepth);
	nglDeleteFramebuffers(2, ctx->hemisphere.fb);
...
void lmSetHemisphereWeights(lm_context *ctx, lm_weight_func f, void *userdata)
{
	...
	nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.firstPass.weightsTexture);
	nglTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, 3 * ctx->hemisphere.size, ctx->hemisphere.size, 0, GL_RG, GL_FLOAT, weights);
	LM_FREE(weights);
}
...
void lmSetTargetLightmap(lm_context *ctx, float *outLightmap, int w, int h, int c)
{
	...
	if (!ctx->hemisphere.storage.texture)
		nglGenTextures(1, &ctx->hemisphere.storage.texture);
	nglBindTexture(GL_TEXTURE_2D, ctx->hemisphere.storage.texture);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	nglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	nglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, w, h, 0, GL_RGBA, GL_FLOAT, 0);
...
//#endif
	fileHandle_t fh = ri.FS_FOpenWrite( filename );
	if ( fh == FS_INVALID_HANDLE ) return LM_FALSE;
	ri.FS_Write( header, sizeof( header ), fh );

	// we make sure to swap it back! trust me. :)
	if (!isGreyscale)
		lm_swapRandBub((unsigned char*)image, w, h, c);
	ri.FS_Write( image, w * h * c, fh );
	if (!isGreyscale)
		lm_swapRandBub((unsigned char*)image, w, h, c);
