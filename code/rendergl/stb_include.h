...
static char *stb_include_load_file(char *filename, size_t *plen)
{
	...
	len = ri.FS_LoadFile( filename, (void **)&text );
	if ( !text || !len ) {
		return NULL;
	}
...
{
	static include_info *z;
	
	z = realloc( array, sizeof( *z ) * ( len + 1 ) );
	z[ len ].offset = offset;
	z[ len ].end = end;
	z[ len ].next_line_after = next_line;
	N_strncpyz( z[ len ].filename, filename, sizeof( z[ len ].filename ) );
...
static void stb_include_free_includes( include_info *infos, int len )
{
	int i;
			...
			if (0==strncmp(s, "include", 7) && stb_include_isspace(s[7])) {
				...
				if (*s == '"') {
					...
					while (*t != '"' && *t != '\n' && *t != '\r' && *t != 0)
						++t;
					if (*t == '"') {
						char *filename = (char *) alloca( t - s + 1 );
...
char *stb_include_strings(char **strs, int count, char *inject, char *path_to_includes, char *filename, char error[256])
{
	...
	text = (char *) ri.Hunk_AllocateTempMemory( length+1 );
	length = 0;
	for (i=0; i < count; ++i) {
