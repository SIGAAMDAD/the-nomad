...
#define IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
#define glBindVertexArray       nglBindVertexArrayOES
#define glGenVertexArrays       nglGenVertexArraysOES
#define glDeleteVertexArrays    nglDeleteVertexArraysOES
...
struct ImGui_ImplOpenGL3_Data
{
    ...
    qboolean        GlProfileIsES2;
    qboolean        GlProfileIsES3;
    qboolean        GlProfileIsCompat;
    GLint           GlProfileMask;
    GLuint          FontTexture;
    GLuint          ShaderHandle;
...
extern "C" ImGui_ImplOpenGL3_Data* ImGui_ImplOpenGL3_GetBackendData()
{
...
// Functions
extern "C" void *ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    GLint major = 0;
    GLint minor = 0;
    nglGetIntegerv(GL_MAJOR_VERSION, &major);
    nglGetIntegerv(GL_MINOR_VERSION, &minor);
    if (major == 0 && minor == 0)
    {
        // Query GL_VERSION in desktop GL 2.x, the string will start with "<major>.<minor>"
        const char* gl_version = (const char*)nglGetString(GL_VERSION);
        sscanf(gl_version, "%d.%d", &major, &minor);
    }
    bd->GlVersion = (GLuint)(major * 100 + minor * 10);
#if defined(GL_CONTEXT_PROFILE_MASK)
    nglGetIntegerv(GL_CONTEXT_PROFILE_MASK, &bd->GlProfileMask);
    bd->GlProfileIsCompat = (bd->GlProfileMask & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) != 0;
#endif

    bd->UseBufferSubData = qfalse;
    /*
    ...
    */
    ...
    nglGetIntegerv(GL_TEXTURE_BINDING_2D, &current_texture);

    // Detect extensions we support
    bd->HasClipOrigin = (bd->GlVersion >= 450);
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_EXTENSIONS
    GLint num_extensions = 0;
    nglGetIntegerv(GL_NUM_EXTENSIONS, &num_extensions);
    for (GLint i = 0; i < num_extensions; i++)
    {
        const char* extension = (const char*)nglGetStringi(GL_EXTENSIONS, i);
        if (extension != nullptr && strcmp(extension, "GL_ARB_clip_control") == 0)
            bd->HasClipOrigin = qtrue;
    }
#endif
