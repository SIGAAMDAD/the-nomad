...
struct block *block_new(size_t page_size) {
    ...
    block->pages = (void **)malloc(sizeof(*block->pages));
    block->offsets = (size_t *)malloc(sizeof(*block->offsets));
...
static void *add_page(struct block *block) {
    if (block->count == block->size) {
        ...
        void **pages = (void **)realloc(block->pages, sizeof(*pages) * new_size);
        if (!pages) {
            return NULL;
        }
        block->pages = pages;
        size_t *offsets = (size_t *)realloc(block->offsets, sizeof(*offsets) * new_size);
        if (!offsets) {
            return NULL;
        }
