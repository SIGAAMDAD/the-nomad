...
struct strings_frequency *strings_frequency_new() {
    struct strings_frequency *frequency = (struct strings_frequency *)malloc(sizeof(*frequency));
    if (!frequency) {
        return NULL;
    }
    frequency->counts = (id_count *)malloc(sizeof(*frequency->counts));
    if (!frequency->counts) {
        free(frequency);
        return NULL;
    }
...
static bool resize_counts(struct strings_frequency *frequency,
    ...
    struct id_count *counts = (struct id_count *)realloc(frequency->counts, sizeof(*counts) * new_size);
    if (!counts) {
        return false;
    }
