...
struct strings *strings_new() {
    struct strings *strings = (struct strings *)malloc(sizeof(*strings));
    if (!strings) {
        return NULL;
    }
...
static tree_node_t *create_node(struct strings *strings, uint32_t hash,
                                const char *string) {
    tree_node_t *node = (tree_node_t *)block_alloc(strings->index, sizeof(*node));
    if (!node) {
        return NULL;
    }
    node->hash = hash;
    ...
    uint32_t *hash_ptr = (uint32_t *)block_alloc(strings->hashes, sizeof(*hash_ptr));
    if (!hash_ptr) {
        return NULL;
    }
    *hash_ptr = hash;
