...
dword idHeap::Msize( void *p ) {
	...
	switch( ((byte *)(p))[-1] ) {
		...
		default: {
			LOG_ERROR( "idHeap::Msize: invalid memory block" );
			return 0;
		}
	}
#endif
...
idHeap::page_s* idHeap::AllocatePage( dword bytes ) {
	...
	if ( swapPage && swapPage->dataSize == bytes ) {			// if we've got a swap page somewhere
		...
		if ( !p ) {
			...
			if ( !p ) {
				N_Error( "AllocatePage: malloc() failure for %i", bytes );
			}
		}
...
void idHeap::SmallFree( void *ptr ) {
	...
	if ( ix > (256 / ALIGN) ) {
		LOG_ERROR( "SmallFree: invalid memory block" );
		return;
	}
