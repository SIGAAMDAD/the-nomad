...
namespace TheNomad::Engine::UserInterface {
	class ConfigSliderVar : ConfigVar {
		...
		ConfigSliderVar( const string& in name, const string& in id, float value, float min, float max ) {
			m_Name = name;
			m_nValue = value;
			m_nMinValue = min;
			m_nMaxValue = max;
		...
		ConfigSliderVar( const string& in name, const string& in id, int value, int min, int max ) {
			m_Name = name;
