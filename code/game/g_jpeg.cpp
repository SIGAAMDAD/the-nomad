...
static void G_JPGErrorExit(j_common_ptr cinfo)
{
...
static void G_JPGOutputMessage(j_common_ptr cinfo)
{
  char buffer[JMSG_LENGTH_MAX];
...
void G_LoadJPG( const char *filename, byte **pic, int32_t *width, int32_t *height )
{
	...
	*/
	...
	len = FS_LoadFile( (char *) filename, &fbuffer.v );
	if ( !fbuffer.b || len < 0 ) {
		return;
	}
	...
	*/
	...
	cinfo.err->error_exit = G_JPGErrorExit;
	cinfo.err->output_message = G_JPGOutputMessage;
    ...
    )
  {
    ...
    N_Error( ERR_DROP, "LoadJPG: %s has an invalid image format: %dx%d*4=%d, components: %d", filename,
...
static boolean empty_output_buffer( j_compress_ptr cinfo )
{
  ...
  N_Error( ERR_FATAL, "Output buffer for encoded JPEG image has insufficient size of %d bytes", dest->size );
...
static void jpegDest (j_compress_ptr cinfo, byte* outfile, int32_t size)
{
  my_dest_ptr dest;
...
=================
*/
size_t G_SaveJPGToBuffer( byte *buffer, size_t bufSize, int32_t quality,
    int32_t image_width, int32_t image_height, byte *image_buffer, int32_t padding)
{
...
void G_SaveJPG( const char *filename, int32_t quality, int32_t image_width, int32_t image_height, byte *image_buffer, int32_t padding )
{
    byte *out;
    size_t bufSize;
    ...
    out = (byte *)Hunk_AllocateTempMemory( bufSize );
	  ...
	  bufSize = G_SaveJPGToBuffer( out, bufSize, quality, image_width, image_height, image_buffer, padding );
