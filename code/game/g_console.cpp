...
typedef struct {
	...
	int		times[NUM_CON_TIMES];	// gi.realtime time the line was generated
...
void Con_ToggleConsole_f( void ) {
    ...
    if ( gi.state == GS_INACTIVE && Key_GetCatcher() == KEYCATCH_CONSOLE ) {
		return;
	}
...
void Con_Init( void ) 
{
	...
	con_xOffset = Cvar_Get ("con_xOffset", "0", 0);
	Cvar_SetDescription( con_xOffset, "Console notifications X-offset." );
	con_color = Cvar_Get( "con_color", "", 0 );
	Cvar_SetDescription( con_color, "Console background color, set as R G B A values from 0-255, use with \\seta to save in config." );
...
//		}
		...
		for (x = 0 ; x < con.linewidth ; x++) {
			...
			SCR_DrawSmallChar( con_xOffset->i + con.xadjust + (x+1)*smallchar_width, v, text[x] & 0xff );
		}
	...
	if ( Key_GetCatcher( ) & KEYCATCH_MESSAGE )
	{
		...
		v /= gi.gpuConfig.vidHeight / 480.0;
...
static void Con_DrawSolidConsole( float frac ) {
	...
	if ( yf < 1.0 ) {
		...
		if ( con_color->s[0] ) {
			...
			re.DrawImage( 0, 0, wf, yf, 0, 0, 1, 1, gi.consoleShader0 );
		}
...
void Con_RunConsole( void ) 
{
	...
	{
		con.displayFrac -= con_conspeed->f * gi.realFrameTime * 0.001;
		if ( con.finalFrac > con.displayFrac )
...
static void Con_DrawSolidConsole( float frac )
{
	...
	refdef.height = gi.gpuConfig.vidHeight * 0.75f;
