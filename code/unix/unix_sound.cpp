...
static qboolean setup_ALSA( smode_t mode )
{
	...
	if ( t_lib == NULL )
	{
		t_lib = Sys_LoadDLL( "libpthread.so.0" );
		if ( t_lib == NULL )
		{
			t_lib = Sys_LoadDLL( "libpthread.so" );
		}
		if ( t_lib == NULL )
		{
			Con_Printf( "Error loading pthread library, disabling ALSA support.\n" );
			return qfalse;
		}
	}

	for ( i = 0 ; i < arraylen( t_list ) ; i++ )
	{
		*t_list[i].symbol = Sys_GetProcAddress( t_lib, t_list[i].name );
		if ( *t_list[i].symbol == NULL )
		{
			Con_Printf( "Couldn't find '%s' symbol, disabling ALSA support.\n",
				t_list[i].name );
			UnloadLibs();
			return qfalse;
		}
	...
	if ( a_lib == NULL )
	{
		a_lib = Sys_LoadDLL( "libasound.so.2" );
		if ( a_lib == NULL )
		{
			a_lib = Sys_LoadDLL( "libasound.so" );
		}
		if ( a_lib == NULL )
		{
			Con_Printf( "Error loading ALSA library.\n" );
			goto __fail;
		}
	}

	for ( i = 0 ; i < arraylen( a_list ) ; i++ )
	{
		*a_list[i].symbol = Sys_GetProcAddress( a_lib, a_list[i].name );
	...
	err = _snd_pcm_open( &handle, s_device->s, SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK );
	if ( err < 0 )
	{
		Con_Printf( "Playback device open error: %s\n", _snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( hwparams == NULL )
	{
		Con_Printf( "Error allocating %i bytes of memory for hwparams\n", (int)_snd_pcm_hw_params_sizeof() );
		goto __fail;
	}
	...
	if ( swparams == NULL )
	{
		Con_Printf( "Error allocating %i bytes of memory for swparams\n", (int)_snd_pcm_sw_params_sizeof() );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Broken configuration for playback: " \
	...
	if ( err < 0 )
	{
		Con_Printf( "Access type not available for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Resampling setup failed for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Sample format not available for playback: " \
			"%s\n", _snd_strerror( err ) );
		goto __fail;
	}
	...
	{
		err = _snd_pcm_hw_params_set_channels( handle, hwparams, channels );
		Con_Printf( "Channels count (%i) not available for playbacks: %s\n",
	...
	if ( err < 0 )
	{
		Con_Printf("Rate %iHz not available for playback: %s\n",
			speed, _snd_strerror( err ) );
		goto __fail;
	}
	if ( rrate != speed )
	{
		Con_Printf( "Rate doesn't match (requested %iHz, get %iHz)\n",
			speed, err );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set period time %i for playback: %s\n",
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set periods (%i): %s",
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to get period size for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set hw params for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to determine current swparams for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set start threshold mode for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set stop threshold for playback: " \
			"%s\n", _snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set avail min for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}
	...
	if ( err < 0 )
	{
		Con_Printf( "Unable to set sw params for playback: %s\n",
			_snd_strerror( err ) );
		goto __fail;
	}

#ifdef INT
	Con_Printf( "period_time=%i\n", period_time );
	Con_Printf( "period_size=%i\n", (int)period_size );
#endif
	...
	if ( snd_async )
	{
		...
		if ( err < 0 )
		{
			Con_Printf( COLOR_YELLOW "ALSA initial write error: %s\n", _snd_strerror( err ) );
			goto __fail;
		}
		if ( err != period_size * 2 )
		{
			Con_Printf( COLOR_YELLOW "ALSA initial write error: written %i expected %li\n", err,
				period_size * 2 );
			goto __fail;
		}
	...
	else
	{
		...
		if ( err != 0 )
		{
			Con_Printf( "Error creating sound thread (%i)\n", err );
#ifdef USE_SPINLOCK
...
{
	//Con_Printf( "...trying ASYNC mode\n" );
	//if ( !setup_ALSA( SND_MODE_ASYNC ) )
	{
		Con_Printf( "...trying MMAP mode\n" );
		if ( !setup_ALSA( SND_MODE_MMAP ) )
		{
			Con_Printf( "...trying DIRECT mode\n" );
			if ( !setup_ALSA( SND_MODE_DIRECT ) )
			{
				Con_Printf( "...ALSA setup failed\n" );
				return qfalse;
			}
		}
	}
...
qboolean SNDDMA_Init( void )
{
	...
	if (!audio_fd) {
		audio_fd = open(snddevice->s, O_RDWR);
	...
	if (rc < 0) {
		perror(snddevice->s);
		Con_Printf("Could not set %s to stereo=%d", snddevice->s, dma.channels);
		close(audio_fd);
		return qfalse;
	}
	...
	if (rc < 0) {
		perror(snddevice->s);
		Con_Printf("Could not set %s speed to %d", snddevice->s, dma.speed);
		close(audio_fd);
		return qfalse;
	}
	...
	if (dma.samplebits == 16) {
		...
		perror(snddevice->s);
		Con_Printf("%d-bit sound not supported.", dma.samplebits);
		close(audio_fd);
		return qfalse;
	}
	...
	{
		Con_Printf("Could not mmap dma buffer PROT_WRITE|PROT_READ\n");
		Con_Printf("trying mmap PROT_WRITE (with associated better compatibility / less performance code)\n");
