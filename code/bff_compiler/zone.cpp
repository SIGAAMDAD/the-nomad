...
#define malloc_aligned(alignment, size) malloc((((size) + ((alignment) - 1)) & ~(((alignment) - 1))))
...
{
	memblock_t* other;
	Con_Printf("Z_MergePB: merging prev block for block at {}", (void *)block);
...
{
	memblock_t* other;
	Con_Printf("Z_MergeNB: merging next block for block at {}", (void *)block);
